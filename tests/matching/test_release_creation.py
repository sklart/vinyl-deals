...
def test_exact_barcode_creates_release_but_possible_does_not(tmp_path: Path) -> None:
    ...
    repository.upsert_offer(RawOffer.now(source="a", source_product_id="1", url="https://a", artist_raw="Artist", title_raw="Album", barcode="4006381333931")); repository.upsert_offer(RawOffer.now(source="b", source_product_id="2", url="https://b", artist_raw="Artist", title_raw="Album", barcode="4006381333931")); build_match_queue(repository); build_match_queue(repository)
