...
def offer(**values): return RawOffer.now(source="test", source_product_id=values.pop("source_product_id", "1"), url="https://example.test", artist_raw=values.pop("artist_raw", "Pink Floyd"), title_raw=values.pop("title_raw", "Animals"), **values)
...
def test_identical_barcodes_tolerate_store_title_variants_but_not_physical_conflicts():
    same = match_offers(
        offer(barcode="4006381333931", title_raw="Communique"),
        offer(source_product_id="2", barcode="4006381333931", title_raw="Communiqué LP 1979"),
    )
    assert same.kind == MatchKind.EXACT_BARCODE
