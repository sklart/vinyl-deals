...
def test_upsert_preserves_price_history(tmp_path: Path) -> None:
    ...
    import sqlite3
    with sqlite3.connect(database) as connection: assert connection.execute("SELECT COUNT(*) FROM offers").fetchone()[0] == 1; assert connection.execute("SELECT COUNT(*) FROM price_history").fetchone()[0] == 2
