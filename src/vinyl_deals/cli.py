...
def main() -> int:
    ...
    scrape = commands.add_parser("scrape", help="Fetch a public store catalogue"); scrape.add_argument("source", choices=["vinyl_ru", "imagine_club", "collectomania"]); scrape.add_argument("--database", type=Path, default=Path("vinyl_deals.sqlite3")); scrape.add_argument("--page-limit", type=int, help="Temporary safe bound for a paginated source")
    ...
    if args.command == "doctor": print("OK: Python runtime available; Vinyl.ru public CSV adapter configured."); return 0
    ...
    if result.state != "active": print("DEGRADED: " + "; ".join(result.warnings)); return 2
    repository = SQLiteRepository(args.database)
    for offer in result.offers: repository.upsert_offer(offer)
