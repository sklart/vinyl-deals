...
class SQLiteRepository:
    ...
    def decide_match(self, offer_id: int, candidate_offer_id: int, decision: str, note: str | None = None) -> None:
        ...
        if decision == "different_release" and shared_release:
            self.rebuild_release_cluster(release_ids[0][0])
    ...
    def create_release_for_pair(self, offer_id: int, candidate_offer_id: int, offer: RawOffer, candidate: RawOffer) -> int:
        ...
        with self._connect() as connection:
            ...
            if left_release and right_release and left_release != right_release:
                manual_same = connection.execute(
                    "SELECT decision FROM manual_match_decisions WHERE offer_id=? AND candidate_offer_id=?",
                    (offer_id, candidate_offer_id),
                ).fetchone()
                manual_same = bool(manual_same and manual_same[0] == "same_release")
                release_id = self.merge_releases(left_release, right_release, connection, allow_critical_conflicts=manual_same)
    ...
    def merge_releases(self, canonical_id: int, redundant_id: int, connection: sqlite3.Connection | None = None, allow_critical_conflicts: bool = False) -> int:
        ...
        try:
            self._validate_release_merge(connection, canonical_id, redundant_id, allow_critical_conflicts)
            self._merge_release_metadata(connection, canonical_id, redundant_id)
    ...
    def _validate_release_merge(self, connection: sqlite3.Connection, canonical_id: int, redundant_id: int, allow_critical_conflicts: bool = False) -> None:
        left = {row[0] for row in connection.execute("SELECT id FROM offers WHERE release_id=?", (canonical_id,))}
        right = {row[0] for row in connection.execute("SELECT id FROM offers WHERE release_id=?", (redundant_id,))}
        conflict = self._manual_different_between(connection, left, right)
        if conflict:
            raise ReleaseMergeConflict(
                f"Cannot merge Release {canonical_id} and {redundant_id}: manual DIFFERENT_RELEASE exists between offers {conflict[0]} and {conflict[1]}."
            )
        ...
        for field in (() if allow_critical_conflicts else ("disc_count", "format", "rpm")):
        ...
        if not allow_critical_conflicts and first_barcode and second_barcode and first_barcode != second_barcode:
    ...
    def rebuild_release_cluster(self, release_id: int) -> None:
        self.initialize()
        with self._connect() as connection:
            ...
            def union(left: int, right: int) -> None:
                ...
                if not self._manual_different_between(connection, left_group, right_group):
